...
def Remove_Hot_Pixels_2D(im, _hotpix_cand, dtype=np.uint16):
    '''Function to remove hot pixels by interpolation in 2D image'''
...
def Find_Hot_Pixels(im, hot_pix_th=0.50, hot_th=4):
    '''Function to remove hot pixels by interpolation in each single layer from a 3D image'''
