...
class DataSet(object):
    ...
    def save_graph_as_gpickle(
        ...
        with open(savePath, 'wb') as _f:
            pickle.dump(graph, _f, pickle.HIGHEST_PROTOCOL)
...
class MERFISHDataSet(ImageDataSet):
    ...
    def get_fiducial_dimension(self, dataChannel):
        ...
        # default to be 2D
        _fiducial_dim = 2 # 2D
        # chagne if the frames are 3D
        if isinstance(_fiducial_frames, np.ndarray) or isinstance(_fiducial_frames, list):
            _fiducial_dim = 3 # 3D
        #print(_fiducial_dim)
