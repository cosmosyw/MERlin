...
class CellPoseSegmentMembrane_Cosmos(FeatureSavingAnalysisTask):
    '''A task that determines the boundaries of features in the
    image data in each field of view using the Cellpose method
    '''
    def __init__(self, dataSet, parameters=None, analysisName=None):
        ...
        if 'nuclear_channel' not in self.parameters:
            self.parameters['nuclear_channel'] = 'DAPI'
        if 'membrane_channel' not in self.parameters:
            self.parameters['membrane_channel'] = 'Na_K_ATPase'
        if 'use_gpu' not in self.parameters:
            self.parameters['use_gpu'] = False
        if 'membrane_diameter' not in self.parameters:
            self.parameters['membrane_diameter'] = 120
        if 'nuclei_diameter' not in self.parameters:
            self.parameters['nuclei_diameter'] = 100
        if 'min_size' not in self.parameters:
            self.parameters['min_size'] = 200
        if 'run_highpass' not in self.parameters:
            self.parameters['run_highpass'] = True
        ...
        if 'dump_segmented_masks' not in self.parameters:
            self.parameters['dump_segmented_masks'] = True
        if 'z_resolution' not in self.parameters:
            self.parameters['z_resolution'] = 500
    ...
    def add_new_segmentation_masks_to_existing_mask(self, existing_mask:np.ndarray, 
        ...
        # Add the cells from the new mask to the existing mask
        current_highest_id = np.max(combined_mask)
        new_cell_ids = np.unique(new_mask)
        new_cell_ids = new_cell_ids[new_cell_ids > 0]
    
        for nc_id in new_cell_ids:
        
            # Only add cells that don't overlap with existing cells
            overlap_size = np.count_nonzero(binary_segemented_mask * (new_mask == nc_id))
        
            if 0 == overlap_size:
                current_highest_id += 1
                combined_mask[new_mask == nc_id] = current_highest_id
    ...
    def filter_membrane_mask_by_nuclei_mask(self, membrane_mask, nuclei_mask, fraction_threshold = 0.15):
        ...
        # get cell ids
        cell_ids = np.unique(membrane_mask)
        # tracker for split
        split_count = 1
        max_cell_id = np.max(cell_ids)
        for cell_id in cell_ids:
            if cell_id!=0:
                # check how many z stacks the current mask spans
                z_indices = np.unique(np.where(membrane_mask==cell_id)[0])
                # Filter out the mask contains only 1 z stack
                if len(z_indices)==1:
                    membrane_mask[np.where(membrane_mask==cell_id)]=0
                # check whether this mask contains many nuclei
                else:
                    current_cell_mask = membrane_mask==cell_id
                    current_mask_with_nuclei = current_cell_mask*nuclei_mask
                    overlapped_cell_ids, overlapped_cell_id_counts = np.unique(current_mask_with_nuclei, return_counts=True)
                    total_number_pixels_current_cell = np.count_nonzero(current_cell_mask)
                    # find uniquely identified cells overlap with the current mask
                    unique_overlap_cells = []
                    for overlap_cell, count in zip(overlapped_cell_ids, overlapped_cell_id_counts):
                        if overlap_cell!=0:
                            # only count the cell with substantial overlap with the current mask
                            if count>=(fraction_threshold*total_number_pixels_current_cell):
                                unique_overlap_cells.append(overlap_cell)
                    # split the mask if more than two cells
                    if len(unique_overlap_cells)>1:
                        # first get rid of the current mask
                        output_mask[np.where(output_mask==cell_id)] = 0
                        for overlap_cell in unique_overlap_cells:
                            new_cell_id = max_cell_id + split_count
                            split_count += 1
                            # get the new mask
                            output_mask[np.where(current_mask_with_nuclei==overlap_cell)] = new_cell_id
        return output_mask
    ...
    def _run_analysis(self, fragmentIndex):
        ...
        # preprocess the images - SKIP, it works better without preprocessing
        # nuclear_images_pp, membrane_images_pp = self.preprocess_image_channels(nuclear_images, membrane_images)
        ...
        # Run the cellpose prediction using the nuclear and membrane stains
        masks_cyto, flows_cyto, styles_cyto, diams_cyto = model_cyto.eval(stacked_images_cyto, 
                                        diameter=self.parameters['membrane_diameter'], 
                                        do_3D=False, channels=[2, 3], 
                                        resample=True, min_size=self.parameters['min_size'])
        ...
        if self.parameters['combine_two_models']:
            ...
            # Load the nuclei model
            model_nuclei = models.CellposeModel(gpu=self.parameters['use_gpu'], model_type='nuclei')

            # Run the cellpose prediction using the nuclear stain
            masks_nuclei, _, _ = model_nuclei.eval(nuclear_images,
                                batch_size=5, anisotropy=self.parameters['z_resolution']/108,
                                cellprob_threshold=0, 
                                channels=[0,0], diameter=self.parameters['nuclei_diameter'], min_size=self.parameters['min_size'],
                                do_3D=True,)

            # filter the cyto2 mask by the nuclei mask
            masks3d = self.filter_membrane_mask_by_nuclei_mask(masks3d, masks_nuclei)

            # Combine the masks from the cyto2 and the nuclei models
            masks_combined = self.add_new_segmentation_masks_to_existing_mask(masks3d, masks_nuclei)

        else:
            masks_combined = masks3d

        
        if self.parameters['dump_segmented_masks']:
            self._save_tiff_images(fragmentIndex, 'segmented_mask', masks_combined)
